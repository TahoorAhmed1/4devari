...
function ProjectDemoCard({ width, selectedValues, images }) {
        ...
        <div className={classes.card_img}>
          ...
          <div className={classes.icons_container}>
            <div className={classes.single_tab}>
              <img src={video_card.src} className={classes.video} alt="Project Promo" />
              <p>
                {selectedValues?.videos?.length > 0
                  ? `${selectedValues.videos.length}`
                  : 0}
              </p>
            </div>
            <div className={classes.single_tab}>
              <img src={camera_card.src} className={classes.camera} alt="Total videos" />
              <p>
                {selectedValues?.images?.length > 0
                  ? `${selectedValues?.images.length}`
                  : 0}
              </p>
            </div>
          </div>
        ...
        <div className={classes.content_container}>
          <div className={classes.content_section}>
            <h2 className={classes.property_title}>
              ...
              {selectedValues.name ? selectedValues.name : `Project Name`}
            </h2>
            <h2 className={classes.price}>
              {selectedValues.priceUnit}{" "}
              {convertNumberToWords(selectedValues.price)}
            </h2>
            {/* <p className={classes.address}>
                {selectedValues.title ? selectedValues.title : `Property Title`}
              </p> */}
            <p className={`${classes.description_text} text-two-line`}>
              {selectedValues?.description || "Description"}
            </p>
            {/* <div className={classes.details_row}>
              <p>
                {selectedValues.noOfBedrooms
                  ? `${selectedValues.noOfBedrooms} Bed`
                  : "0 Bed"}
              </p>
              <p>
                {selectedValues.noOfBathrooms
                  ? `${selectedValues.noOfBathrooms} Bath`
                  : "0 Bath"}
              </p>
              <p>
                {selectedValues.areaSize ? `${selectedValues.areaSize}` : "0"}{" "}
                {selectedValues.areaSizeUnit}
              </p>
            </div> */}
